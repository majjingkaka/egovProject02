		...
		try{
			if( !oNavigator.ie ){
				if(!!oNavigator.safari && oNavigator.version <= 5){
					...
					bSupportDragAndDropAPI = true;//true
				}
			} else {
				bSupportDragAndDropAPI = false;
			}
    ...
    function html5Upload() {	
    	...
    	//sUploadURL= '/se2upload/editFileUploader2.do'; 	//upload URL
    	//sUploadURL= '/se2823/sample/photo_uploader/file_uploader_html5.jsp';
    	sUploadURL= '/se2823/sample/photo_uploader/file_uploader_html5.jsp';
    ...
    function callAjaxForHTML5 (tempFile, sUploadURL){
		...
		oAjax.header("file-name",tempFile.name); //encodeURIComponent(tempFile.name)
    ...
    function callAjaxForHTML5_22 (tempFile, sUploadURL){
    	...
    	//var token = document.getElementById("_csrf");
    	//var header = document.getElementById("_csrf_header");
		...
		/*
		var form = new ForData();
		form.append("file1", tempFile)
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			sUrl  : "/se2823/sample/photo_uploader/file_uploader.jsp", //location.href.replace(/\/[^\/]*$/, '') + '/file_uploader.php',	//샘플 URL입니다.
 	        sCallback : "/se2823/sample/photo_uploader/callback.jsp", //location.href.replace(/\/[^\/]*$/, '') + '/callback.html',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
